...
const ui = {
  ...
  async preencherFormulario(pensamentoId) {
    ...
    document.getElementById("pensamento-data").value = data.toISOString().split("T")[0]
  ...
  async renderizarPensamentos(pensamentosFiltrados = null) {
    ...
    try {
        let pensamentosParaRendenizar

        if(pensamentosFiltrados){
          pensamentosParaRendenizar = pensamentosFiltrados
        }
        else{
          pensamentosParaRendenizar = await api.buscarPensamentos() 
        }

      if (pensamentosParaRendenizar.length === 0) {
        mensagemVazia.style.display = "block"
      } else {
        mensagemVazia.style.display = "none"
        pensamentosParaRendenizar.forEach(ui.adicionarPensamentoNaLista)
      } 
    }
    catch {
  ...
  adicionarPensamentoNaLista(pensamento) {
    ...
    iconeFavorito.alt = 'botão favoritar'
